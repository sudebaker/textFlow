...
def extract_with_docling(file_path):
    ...
    try:
        ...
        num_pages = len(doc_result.document.pages) if hasattr(doc_result.document, 'pages') else 1
...
def extract_text_from_pdf_raw(file_path):
    ...
    try:
        ...
        if not text_parts:
            readable = ''.join(chr(b) for b in pdf_data if 32 <= b <= 126 or b in (9, 10, 13))
...
async def convert_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✓ Extracted {len(markdown_text)} chars using {extraction_method}")
    ...
    try:
        ...
        # Docling convert() requires a file path, so write to temp file first
        import tempfile
        from pathlib import Path
        ...
        try:
            # Convert using convert method with file path
