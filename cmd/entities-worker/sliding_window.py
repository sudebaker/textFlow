...
GLINER_MAX_TOKENS = int(GLINER_MAX_WORDS / WORDS_PER_TOKEN_RATIO)  # ~288 tokens as safe estimate
...
def positions_overlap(entity1: Dict[str, Any], entity2: Dict[str, Any], buffer: int = OVERLAP_REGION_BUFFER) -> bool:
    """
    ...
    for entity in all_entities:
        ...
        for merged_key, merged_entity in list(merged.items()):
            if entities_are_duplicate(entity, merged_entity):
                ...
                merged_score = merged_entity.get("score", merged_entity.get("confidence", 0))
...
def requires_sliding_window(text: str, threshold_tokens: int = WINDOW_TOKEN_SIZE) -> bool:
    """
