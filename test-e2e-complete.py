...
def upload_document(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        response = requests.post(
            f"{ORCHESTRATOR_URL}/v1/documents/process",
            json=payload,
            timeout=30
        )
...
def get_job_status(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        response = requests.get(
            f"{ORCHESTRATOR_URL}/v1/documents/{job_id}",
            timeout=10
        )
    ...
    try:
        ...
        if 'entities' in results:
            ...
            for entity in entities[:5]:
                print(f"      • {entity.get('text')} ({entity.get('label')}) - confidence: {entity.get('confidence', 0):.2f}")
        
        # Display embeddings
        if 'embeddings' in results:
            embeddings = results['embeddings']
            if isinstance(embeddings, list):
                print(f"\n🔢 Embeddings: {len(embeddings)} dimensions")
...
def save_results_to_file(job_id: str, results: Dict[str, Any]) -> bool:
    ...
    """
    output_dir = Path("/home/amphora/Proyectos/ia-text-ochestrator/data/output")
...
def main():
    ...
    if not results:
        print("\n❌ Test failed: Could not download results")
        return 1
