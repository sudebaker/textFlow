...
Data Results:
  :text → extracted text (string, UTF-8)
  :chunks → JSON array of chunk objects
    [{"text": "...", "index": 0, "source": "page_1"}, ...]
  :embeddings → JSON float32 vector (1024-dim for bge-m3)
...
Results Aggregation:
  :results → Full JobResults JSON (generated by completion worker)
...
**TTL (Time-To-Live)**: All keys expire after 24 hours (default `JOB_TTL=24h`)
